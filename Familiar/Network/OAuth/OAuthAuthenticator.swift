...
//            urlRequest.headers.add(name: "ACCESS_TOKEN", value: credential.accessToken)
    ...
    func refresh(_ credential: OAuthCredential,
        ...
        let request = session.request(AuthRouter.tokenRefresh(refreshToken: refreshToken))

        request.responseDecodable(of: TokenData.self){ result in
            switch result.result {

            case .success(let value):

                // 재발행 받은 토큰 저장
                UserDefaultsManager.shared.setTokens(accessToken: value.access ?? "",
                                                     refreshToken: value.refresh ?? "")

                let expiration = Date(timeIntervalSinceNow: 60 * 60)

                // 새로운 크리덴셜
                let newCredential = OAuthCredential(accessToken: value.access ?? "",
                                                    refreshToken: value.refresh ?? "",
